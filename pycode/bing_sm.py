...
class BingSM(Screen):
    ...
    def crawl_all(self):
        while len(self.new_urls):
            ...
            requests.exceptions.InvalidSchema,requests.exceptions.ChunkedEncodingError, requests.exceptions.ChunkedEncodingError("Connection broken: InvalidChunkLength(got length b'', 0 bytes read)"),requests.exceptions.ChunkedEncodingError("got length b'', 0 bytes read")):
