...
class haberler:
    ...
    def news_getLink(self,url,keyword):
        r = requests.get(url)  
    ...
    def main(start_date,end_date,keyword):
        ...
        dateLinks = haberler.date_creator(start_date, end_date)  # tarih
        for  i in dateLinks:
            newsLinks.append(haberler.getAllLinks(i))
        print("Creating links")
        for j in range(0,len(newsLinks)):
            t1 = time.time()
            with concurrent.futures.ProcessPoolExecutor() as execut:
                b_res = [execut.submit(
                    haberler.news_getLink, i.strip(), keyword) for i in newsLinks[j]]
        ...
        with concurrent.futures.ProcessPoolExecutor() as execut:
            b_res = [execut.submit(
                haberler.creator, i.strip(),keyword) for i in newsLinksAll]
        print(time.time()-t3)
